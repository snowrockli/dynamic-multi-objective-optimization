function cell=cell_neural_tm_re(t,dt,i,j,cell,OD,theita,N_agent_length)
if i~=j
    ...
    for ii=1:N_agent_length
        for jj=1:N_agent_length
            ...
            cell{i,j}.cell{ii,jj}.car_regret(dt)=cell{i,j}.car_fee(dt)-1+exp(-cell{i,j}.cell{ii,jj}.risk*(min(min_bus_fee,cell{i,j}.car_fee(dt))-cell{i,j}.car_fee(dt)));
            %========公交车后悔值=====================
            for k=1:cell{i,j}.route_num
                cell{i,j}.cell{ii,jj}.bus_regret(k,dt)=cell{i,j}.bus_fee(k,dt)-1+exp(-cell{i,j}.cell{ii,jj}.risk*(min(min_bus_fee,cell{i,j}.car_fee(dt))-cell{i,j}.bus_fee(k,dt)));
            end
        end
    end
    if t==1
        for ii=1:N_agent_length
            for jj=1:N_agent_length
                ...
                %========计算出行方式选择概率========================
                cell{i,j}.cell{ii,jj}.input=[cell{i,j}.cell{ii,jj}.bus_regret(1:end,dt);cell{i,j}.cell{ii,jj}.car_regret(dt);cell{i,j}.departure_fee(dt)]';%神经网络的输入
                ...
                if cell{i,j}.cell{ii,jj}.sum_sig==0%防止异常值
                    cell{i,j}.cell{ii,jj}.sig_output=(1/output_num)*ones(1,output_num);
                end
                cell{i,j}.cell{ii,jj}.sum_sig=sum(cell{i,j}.cell{ii,jj}.sig_output);
    ...
    for ii=2:N_agent_length-1
        for jj=2:N_agent_length-1
            ...
            %=======学习交互==============
            cell{i,j}.cell{ii,jj}.risk=(1-theita)*cell{i,j}.cell{ii,jj}.risk+theita*cell{i,j}.cell{best_x(1),best_y(1)}.risk;
